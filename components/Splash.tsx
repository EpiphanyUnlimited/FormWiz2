...
import { Mic, FileText, CheckCircle, ArrowRight, Moon, Sun, Shield, Info } from 'lucide-react';
...
const Splash: React.FC<SplashProps> = ({ onProceed, onPrivacy, onInstructions, darkMode, toggleTheme }) => {
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-600 to-indigo-800 dark:from-slate-900 dark:to-slate-800 flex flex-col justify-between p-6 font-sans text-white transition-colors duration-300">
      ...
      {/* Footer Links */}
      <div className="w-full max-w-4xl mx-auto flex justify-center gap-8 mt-8 text-sm text-blue-200/60">
        <button onClick={onPrivacy} className="flex items-center gap-2 hover:text-white transition-colors">
            <Shield size={14} /> Privacy Policy
        </button>
        <button onClick={onInstructions} className="flex items-center gap-2 hover:text-white transition-colors">
