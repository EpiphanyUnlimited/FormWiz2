...
function App() {
    ...
    const handleUpgradePlan = (newPlan: PlanType) => {
        if (!user) return;
        // Optimistically update UI, but real source of truth should be backend webhook in a real app
        // For now we trust the client after success callback
    ...
    const handleSplashProceed = () => {
        if (user) {
            ...
            // Trigger Netlify Identity Widget
            auth.signup();
        }
    };

    // Deprecated: HandleLogin is now managed by Netlify Identity events
    // kept only if referenced elsewhere, but better to remove usage

    const handleLogoutCleanup = () => {
        setUser(null);
        // Note: We don't remove autoform_user from localStorage as we rely on Netlify Identity cookie now
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        if (e.target.files && e.target.files[0]) {
            ...
            try {
                ...
                // We check using a combination of MIME type and extension to be robust
                            ...
                            <div className="flex flex-col items-center justify-center w-80 h-64 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-2xl bg-white dark:bg-slate-800 hover:border-blue-500 dark:hover:border-blue-400 hover:bg-blue-50 dark:hover:bg-slate-750 transition-all duration-300 shadow-sm">
                                {isConverting ? (
                                    ...
                                    <>
                                        ...
                                        <p className="text-xs text-slate-400 mt-2 text-center">Supported: PDF, JPG, PNG, Word</p>
                                    </>
                                )}
                            </div>
